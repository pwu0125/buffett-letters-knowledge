...
| `buffet_return_data.csv` | 首页 Hero 收益图数据源 | 覆盖 1957–2025：1957–1964 合伙基金收益、1965–2025 每股账面价值变动（2019–2025 按各年 10-K 的 Berkshire 股东权益 ÷ A 股等值股本推算，见下）；连续复利，图表注释写明口径，不要删除 2019 年后数据、也不要改用市场价值口径 |
...
- `buffet_return_data.csv` 当前字段为 `年份,第N年,年收益率%,模拟累计净值,年化收益%`，覆盖 1957–2025 共 69 行：1957–1964 为合伙基金收益率，1965–2025 为伯克希尔每股账面价值变动率。1965–2018 取自历年披露的每股账面价值；2019–2025 年报不再披露每股账面价值，按各年 10-K 的 Berkshire 股东权益（StockholdersEquity，归属母公司）÷ 年末 A 股等值股本（10-K 注释「on an equivalent Class A common stock basis, there were N shares outstanding as of December 31」）推算，逐年复核 2018 锚点（+0.36% ≈ 既有 +0.4%）以保证口径连续。按年连续复利；`年化收益%` = 累计净值^(1/第N年) − 1。调整时核对年份、累计净值和年化收益之间的一致性，并同步图表口径注释；不要改用每股市场价值口径。
...
- 收益 CSV 为 1957–2025 共 69 行数据（1965–2025 统一每股账面价值口径，连续复利）；
