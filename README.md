...
- **AI 讨论**：每篇文章旁聊天面板；**回答前会自动预检索全库并用工具深入**（全库 188 篇检索 / 精读指定文章 / 五维分类索引 / 权威收益数据 / 联网检索与网页抓取），不再是"只读当前这篇"；每条 AI 答复可一键「加入笔记」；OpenAI 兼容接口（默认 DeepSeek）
- **与巴菲特对话 / 与芒格对话**：基于 `celebrity-buffett` / `celebrity-charlie-munger` 人格（distilly 蒸馏，项目内 `skills/` 下）的双对话专栏，主页双入口（巴菲特头像 / 芒格头像），护城河、多元思维模型等框架先研究再回答；**同样具备全库检索 + 联网核实能力**；两套对话记忆独立持久化
- **问答 AI 的作答标准（研究工作流）**：先查证再回答（数字一律用工具取数，不凭印象）→ 交叉验证（关键事实两个来源，口径冲突必须说明，如每股账面价值 vs 每股市场价值）→ 标注口径/区间/算法 → 不编造（语料没有就明说并标注推演）→ 输出「结论先行 / 证据链 / 口径与局限 / 可操作建议」四段结构；每条答复末尾附**研究过程**（实际调用了哪些工具、什么参数、是否命中），便于复核
- **主页 Hero 交互图**：巴菲特历年表现（1957–2025；1957–1964 为合伙基金收益，1965–2025 统一为伯克希尔每股账面价值变动率——2019 年起年报不再披露该指标，按各年 10-K 的股东权益 ÷ 年末 A 股等值股本推算）——年度变动率柱状图（正琥珀/负深红）+ 累计净值对数轴曲线 + 年化收益虚线，十字线 tooltip、图例开关、滚轮/滑块缩放，右上角标注口径；数据来自 `buffet_return_data.csv`（官方年报/SEC 数据核对 + 复算校验）。
...
问答 AI（文章讨论 / 巴菲特对话 / 芒格对话）通过**工具调用**获取证据，工具由前端执行，结果回灌模型继续推理（最多 5 轮）：

| 工具 | 作用 |
| --- | --- |
| `search_articles` | 在全部 188 篇文章中检索关键词，返回文章与命中片段 |
| `read_article` | 精读指定文章全文（可分段） |
| `search_index` | 取某年信件摘要/市场事件/主题/原文链接，或检索主题/行业/事件/方法维度 |
| `get_returns` | 读取 1957–2025 权威收益序列与负增长年份（含口径说明） |
| `web_search` | 联网检索（标题/网址/摘要） |
| `fetch_url` | 抓取网页正文（自动去脚本/样式、识别压缩与 PDF） |

- **联网检索配置**（二选一，Key 都不落盘）：
  - 应用「设置」面板 → 联网检索 → 选择提供方（博查 Bocha / Tavily）并填写搜索 API Key（只存本机浏览器 localStorage）；
  - 或设置环境变量 `BOCHA_API_KEY` / `TAVILY_API_KEY`（也可写入项目根 `.env`）后启动本地服务。
- 联网检索与网页抓取由**本地服务代理**执行（`/api/websearch`、`/api/fetch`），因此需要以服务模式运行（Mac/Windows 应用默认如此；`file://` 直开时为离线模式，联网工具会返回提示）。
- 抓取默认拒绝内网/本机地址（防 SSRF）；若本机走企业代理导致公网域名解析为内网地址，可设 `BUFFETT_FETCH_ALLOW_PRIVATE=1` 放开。站点返回 Brotli 压缩时需 `python3 -m pip install brotli`（可选依赖，缺失时给出明确提示而不是返回乱码）。
